...
//        Uri uri = uriBuilder.build();
        ...
        for (ResolveInfo resolveInfo : resolveInfoList) {
            try {
                ...
                // One app's icon/label failed (e.g. a corrupt package) — skip
                // that app instead of aborting the whole list. Aborting here
                // used to fail EVERY getAllUpiApps call on API 26+ devices,
                // where adaptive icons made getBitmapFromDrawable throw
                // before the fix below.
                Log.d(TAG, "Skipping UPI app: " + e);
            }
        }
    ...
    private Bitmap getBitmapFromDrawable(Drawable drawable) {
        // Adaptive icons (API 26+) report intrinsicWidth/Height == -1, which
        // made createBitmap throw and — with the old per-app catch — abort
        // the WHOLE getAllUpiApps call. Draw onto a fixed 48x48 canvas when
        ...
        if (uniqueRequestCode == requestCode && finalResult != null) {
            if (data != null) {
                try {
                    ...
                    // Some UPI apps return data without the response string
                    // (and never RESULT_CANCELED) — treat that as a
                    // cancellation. Reporting success(null) used to crash
                    // UpiResponse on the Dart side and map a possibly-paid
                    // transaction to a generic failure.
